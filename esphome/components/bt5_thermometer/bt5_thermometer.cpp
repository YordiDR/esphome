...
void BT5Thermometer::gattc_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if,
  ...
  switch (event) {
    case ESP_GATTC_SEARCH_CMPL_EVT: {
      ESP_LOGD(TAG, "Service discovery complete for BT5 Thermometer!");
    ...
    case ESP_GATTC_CONNECT_EVT: {
      ESP_LOGI(TAG, "Connected to BT5 Thermometer");
      if (this->connection_sensor_ != nullptr) {
    ...
    case ESP_GATTC_DISCONNECT_EVT: {
      ESP_LOGI(TAG, "Disconnected from BT5 Thermometer");
      if (this->connection_sensor_ != nullptr) {
        this->connection_sensor_->publish_state(false);
      }
      break;
    ...
    case ESP_GATTC_NOTIFY_EVT: {
      ESP_LOGD(TAG, "BLE Notify received on handle: %d", param->notify.handle);
      if (param->notify.handle == this->char_handle_) {
        ESP_LOGD(TAG, "BLE Notify received for BT5 thermometer, parsing response...");
...
void BT5Thermometer::parse_data_(const uint8_t *data, uint16_t length) {
  for (BT5ProbeSensor *probe : this->probes_) {
    ...
    if (low_byte_id < length) {
      ...
      ESP_LOGD(TAG, "Parsed Probe %d raw temperature: %d", probe_num, raw_temp);
      if (raw_temp != 65535) {
