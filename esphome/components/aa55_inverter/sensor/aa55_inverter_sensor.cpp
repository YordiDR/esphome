...
#include "esphome/components/aa55_inverter/sensor/aa55_inverter_sensor.h"

namespace esphome {
namespace aa55_inverter {
static const char *LOGGING_TAG = "aa55_sensor";

AA55InverterSensor::AA55InverterSensor(std::string id, aa55_const::SENSOR_TYPE type, uint16_t skip_updates,
...
void AA55InverterSensor::process_response(const std::vector<uint8_t> &payload) {
  ...
  if (this->time_to_update()) {
    ...
    this->publish_state(this->parse_int(payload) /
...
void AA55InverterSensor::handle_inverter_offline() {
...
void AA55InverterSensor::dump_config() {
